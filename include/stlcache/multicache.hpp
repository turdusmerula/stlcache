...
#ifndef STLCACHE_MULTICACHE_HPP_INCLUDED
#define STLCACHE_MULTICACHE_HPP_INCLUDED
...
namespace stlcache {
    ...
    /*! \brief MultiCache is a kind of a multimap that have limited number of elements to store and configurable policy for autoremoval of excessive elements.
     *  
     * The multicache behaviour resembles a std::multimap behaviour (but not yet fully compatible with usual std::multimap interface), but it adds a limitation of
     ...
     *  
     * Internally it is built as a wrapper over a std::multimap with some additional checking for cache size and entry removal logic. Unfortunately not all interfaces
     * of std::multimap are supperted by now, but it is a work in progress.
     *  
     * You can use the multicache in a simple and obvious way:
     * \code 
     *     multicache<string,string,policy_lru> cache_lru(3);
     ...
     * You could \link cache::insert insert \endlink as many entries as you like, even more entries, that cache instance can hold.
     * When the cache starts to overflow, it will automatically remove entries, depending on specified Policy.
     ...
     * \tparam <Container> A specialiser for internal container that allow to change how the map stores elements
     *  
     * So, the full example of cache instantiation will be: 
     * \code 
     *     multicache<int,string,policy_none,container_unordered_multimap> cache_none(100500);
     * \endcode
     *  
     * \see policy 
     *  
     * \author chollya (5/19/2011)
     * \author sbesombe (14/01/2016)
     */
    template<
        class Key, 
        class Data, 
        class Policy, 
        class Container = container_unordered_multimap
    >
    class multicache {
    	using container_type = typename Container::template bind<Key,Data> ;
		using storage_type = typename container_type::multimap_type ;
    ...
    public:
        ...
        /*! \name std::multimap interface wrappers
         *  Simple wrappers for std::multimap calls, that we are using only for mimicking the map interface
         */
        ...
        /*! \brief storage object accessor
          ...
          *
          *  \return The storage object of type std::multimap.
          */
        ...
        /*! \brief Counts entries with specified key in the cache.
          *  
          *  Provides information on number of cache entries with specific key.
          *  The difference between this member and \link cache::check check member \endlink is that this one returns number of keys
          *  without touching the entry usage count.
          *  
          *  \param <x> key to count
          *  
          *  \return Number of objects with specified key in the cache (0 for non-existent object)
          *
          *  \see check
          *  
          */        
        ...
        /*! \brief Value comparison object accessor
         *  
        ...
        /*! \brief Key comparison object accessor
          * 
         ...
         * \see cache::operator= 
         */
        void swap ( multicache<Key,Data,Policy,Container>& mp ) throw(exception_invalid_policy) {
         ...
         *  
         * \return number of removed entries or zero when nothing was done.
         */
        ...
        bool insert(Key _k, Data _d) throw(exception_cache_full,exception_invalid_key) {
            ...
            _storage.insert(value_type(_k,_d)) ;
            _currEntries++;
         ...
         * construction time and cannot be modified (with exception of \link cache::swap swap operation \endlink)
         *  
         ...
         *
         * Notice that this function fetches a single element (of the possibly multiple elements with equivalent keys).
         * TODO: To obtain the entire range of equivalent elements, see multimap::equal_range.
         *
         ...
         *  
         *  Tests, whether key exists in the cache or not. During check it's usage count is increased, as opposed to \link cache::count count member \endlink
         *  
         ...
         *  \param <x> a cache object with the same template parameters 
         */
        multicache(const multicache<Key,Data,Policy,Container>& x) throw() {
            *this=x;
        }
        /*!
         ...
         * 
         */
        explicit multicache(const size_type size) throw() {
...
#endif /* STLCACHE_MULTICACHE_HPP_INCLUDED */
